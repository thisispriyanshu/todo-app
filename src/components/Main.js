...
const Main = ({ todoList, setTodoList }) => {
	...
	const columns = [
		...
		},
		{
			...
			render: (due_date) =>
				due_date ? (
					<div className='tag flex items-center bg-sky-400 rounded-md text-white p-2 py-1 cursor-default w-max'>
						{due_date}
					</div>
				) : (
					<div className='tag flex items-center bg-sky-400/10 rounded-md text-sky-400 p-2 py-1 cursor-default w-max'>
						--
					</div>
				),
		},
					...
					<ul className='flex items-center gap-2 flex-wrap m-0'>
						{data ? (
							data.map((tag, index) => (
								<li
									key={index}
									className='tag flex items-center bg-sky-400 rounded-md text-white p-2 py-1 cursor-default'>
									<span className='tag-title capitalize'>{tag}</span>
								</li>
							))
						) : (
							<li className='tag flex items-center bg-sky-400/10 rounded-md text-sky-400 p-2 py-1 cursor-default'>
				...
				return (
					<div className='flex gap-2'>
						<div
							...
							className='cursor-pointer border w-max p-2 bg-sky-400/10 hover:bg-sky-400/30 rounded-full'>
							<MdDelete size={15} />
						</div>
						<div
							title='Edit'
