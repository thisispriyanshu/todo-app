...
const AddTodo = ({ setAddTodo, addTodo, todoList, setTodoList }) => {
	...
	return (
		<div className='addTodo absolute top-0 left-0 min-h-screen grid z-50 min-w-full bg-sky-400/50 place-items-center'>
			<div className='m-2 modal shadow-2xl bg-white p-4 rounded flex items-start flex-col gap-4'>
				...
				<div
					className={`${
						!formValidity
							? 'bg-sky-400/50 cursor-not-allowed pointer-events-none'
							: 'bg-sky-400/90 cursor-pointer'
					}  w-full py-2 hover:bg-sky-400 text-white rounded `}
					onClick={onSubmit}>
